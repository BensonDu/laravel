...
(function () {
  ...
  (function () {
    ...
    parent.imageCrop = function (file,opts) {
    ...
    this.domInit = function () {
      var con = $('#uploadContainer');
      if(!con.length){
        $('body').append(self.template());
        image       = document.getElementById('croppedImage');
        container   = $('#uploadContainer');
        confirm = $('#uploadConfirm').click(self.confirm);
        $('#uploadCancel').click(self.cancel);
      }
      else{
        con.addClass('active');
      }
    ...
    this.cancel = function () {
      return image.src = '', container.removeClass('active');
    };
    this.getImageData = function (a,b) {
      var c, d;
      if ($.isFunction(b))
        return (window.FileReader && FileReader.prototype.readAsDataURL && /^image/.test(a.type)) ? (c = new FileReader, c.onload = function(a) {return b(a.target.result)}, c.readAsDataURL(a),(type = a.type),(isGif = /gif/.test(a.type))) : b('')
    };
    this.cropper = null;
    this.cropperInit = function () {
      if(self.cropper)self.cropper.destroy();
      return (option.croppedable && !isGif) && (self.cropper = new Cropper(image, {
            built: function () {
    ...
    this.confirm = function () {
      ...
      if(!option.croppedable || isGif){
        self.uploadNormal();
      }
      else{
        self.uploadBase64(self.croppedImage());
      }
    };

    this.croppedImage = function () {
      return self.cropper.getCroppedCanvas().toDataURL(type);
    };
    this.put64 = function(img,token){
      var pic = img;
      ...
      xhr.onreadystatechange=function(){
        ...
        if (xhr.readyState==4){
          ...
          if(data.key)self.finish(base+data.key);
    ...
    this.uploadBase64 = function (image) {
      ...
      request.get('/qiniu/upload/token',function (token) {
        if(token.token){
          self.put64(b,token.token);
        }
      });
    };
